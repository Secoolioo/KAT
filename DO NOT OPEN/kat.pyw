# -*- coding: utf-8 -*-
"""
KAT - keeps the Windows session "active" without breaking anything.

Behavior:
  * While you are using the PC (real mouse/keyboard input), KAT does NOTHING.
  * Only after a configurable idle time (IDLE_THRESHOLD) with no real input does
    KAT become active: every ~30 seconds it nudges the mouse a few pixels (and
    moves it back), occasionally taps the harmless F15 key and scrolls minimally.
  * As soon as real input arrives again, KAT pauses automatically and waits for
    the idle time once more.

No window. Control only via the tray icon (possibly under "hidden icons"):
right-click -> Pause / Resume / Quit.

NO logging, no automatic messages: KAT never records when it ran.
Python standard library only (ctypes) -> no extra packages required.
"""
...
# ------------------------------ Configuration ------------------------------
IDLE_THRESHOLD = 120       # sec. without real input before KAT becomes active (2 min)
ACTION_MIN     = 25        # min. seconds between two actions
ACTION_MAX     = 35        # max. seconds between two actions
JIGGLE_PIXELS  = 3         # how far the mouse nudges (pixels)
KEY_CHANCE     = 0.30      # probability per action for an F15 key press
SCROLL_CHANCE  = 0.15      # probability per action for a tiny scroll
ENABLE_CLICK   = False     # REAL left clicks. WARNING: may click whatever is under
                           # the cursor -> OFF by default. Set to True if desired.
CLICK_CHANCE   = 0.10      # only effective when ENABLE_CLICK = True
TRAY_TOOLTIP   = "KAT"     # hover text on the tray icon (change freely)
NOTIFY_ON_ACTIVE = False   # NO balloon when auto-activating (while you are away)
NOTIFY_ON_START  = False   # no start/click notice window (silent). True only for testing.
MAGIC          = 0x4B415431  # signature of our own input events ("KAT1")
# ---------------------------------------------------------------------------

SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
ICON_PATH  = os.path.join(SCRIPT_DIR, "kat.ico")

# Intentionally NO logging: KAT never records when it ran or was active.
...
def do_action():
    """Perform an inconspicuous, harmless activity."""
    p0 = wt.POINT()
    ...
    _send(_mouse(dx, dy))                 # a few pixels...
    time.sleep(0.04)
    p1 = wt.POINT()
    user32.GetCursorPos(ctypes.byref(p1))
    _send(_mouse(p0.x - p1.x, p0.y - p1.y))   # move back exactly (no drift, even at screen edge)

    if random.random() < KEY_CHANCE:      # harmless key, types no characters anywhere
        _send(_key(VK_F15), _key(VK_F15, up=True))

    if random.random() < SCROLL_CHANCE:   # tiny scroll, net zero
...
def status_text():
    ...
    if idle >= IDLE_THRESHOLD:
        return "active - keeping awake (last action %s, %dx)" % (_last_action_str, _action_count)
    rest = int(IDLE_THRESHOLD - idle)
    return "waiting for idle (%d:%02d left, %dx total)" % (rest // 60, rest % 60, _action_count)
...
def _notice(text):
    # Unmissable control message only on YOUR start/click (not while you are away).
    # Runs in its own thread so the message loop is not blocked.
...
def show_menu(hwnd):
    ...
    user32.AppendMenuW(menu, MF_STRING, ID_TEST, "Test now (run one action)")
    user32.AppendMenuW(menu, MF_STRING, ID_STATUS, "Show status")
    user32.AppendMenuW(menu, MF_STRING, ID_TOGGLE,
                       "Resume" if _paused else "Pause")
    user32.AppendMenuW(menu, MF_SEPARATOR, 0, None)
    user32.AppendMenuW(menu, MF_STRING, ID_EXIT, "Quit")
    pt = wt.POINT()
    ...
    if cmd == ID_TOGGLE:
        _paused = not _paused
        if not _paused:
            _mark_user_activity()   # after resuming, wait for the idle period again
        update_tooltip()
    elif cmd == ID_TEST:
        do_action()
        _action_count += 1
        ...
        show_balloon("KAT - Test",
                     "Action performed: the mouse moved briefly. KAT works.")
    elif cmd == ID_STATUS:
...
def py_wndproc(hwnd, msg, wparam, lparam):
    ...
    if msg == WM_REFRESH:
        ...
        if _want_balloon:
            _want_balloon = False
            show_balloon("KAT", "is now keeping your status active.")
        return 0
    if msg == WM_PING:
        _notice("Trace host already running.")
        return 0
...
def main():
    ...
    if ctypes.get_last_error() == 183:   # ERROR_ALREADY_EXISTS
        # Already running -> notify the running instance (feedback on double-click)
    ...
    # Install low-level hooks on this (message-pumping) thread
    ...
    if not _mouse_hook or not _kbd_hook:
        # Input detection failed -> better not run at all, otherwise KAT could
        # become active while the user is working.
        if _mouse_hook:
